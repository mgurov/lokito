...
import { LogDataState, LogIndexNode } from './logDataSlice';
...
//TODO: move from LogIndexNode to string with the id perhaps.
function updateTraceIdIndex(traceIdIndex: Record<string, LogIndexNode[]>, newRecords: Log[]) {
  for (const rec of newRecords) {
    const newValueToAdd: LogIndexNode = {
      id: rec.id,
      stream: {},
      sourceIds: [''],
    }
    ...
    for (const {traceIdValue} of traceIdFields(rec)) {
      if (traceIdIndex[traceIdValue]) {
        traceIdIndex[traceIdValue].push(newValueToAdd)
      } else {
        traceIdIndex[traceIdValue] = [newValueToAdd]
      }
    }    
  }
}
