...
test.describe("continue from where stopped", () => {
  ...
  test(
    "then, we should apply stored value to the first fetch moment",
    AnnotationSuppressDefaultApp,
    async ({ mainPage, logs, appState }) => {
      ...
      ).toStrictEqual([s1fetch, s2fetch, "2025-02-05T08:30:00.000Z"].map(s => s.substring(0, 19)));
    },
  );
  ...
  test(
    "should not show continue when we stopped if no active source last fetch recorded",
    AnnotationSuppressDefaultApp,
    async ({ mainPage, appState }) => {
      const [_activeSource, inactiveSource] = await appState.givenSources({ active: true }, { active: false });

      await appState.storage.setLocalItem(
        SourceLocalStorage.lastSuccessFrom.key(inactiveSource.id),
        "2025-02-04T16:30:00.000Z",
        true,
      );
  ...
  test(
    "should indicate how early would be fetching from",
    AnnotationSuppressDefaultApp,
    async ({ mainPage, appState }) => {
      const [s1, s2] = await appState.givenSources({}, {});

      await appState.storage.setLocalItem(
        SourceLocalStorage.lastSuccessFrom.key(s1.id),
        "2025-02-04T16:30:00.000Z",
        true,
      );
      await appState.storage.setLocalItem(
        SourceLocalStorage.lastSuccessFrom.key(s2.id),
        "2025-02-04T16:35:00.000Z",
        true,
      );
